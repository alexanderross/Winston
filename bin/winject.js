...
- De-Winston this thing. Winston is dead to me.
...
var Winston = {
    ...
    loadWrapper: function( wrapper){
        ...
        $(document.getElementById("winston_AEC")).removeClass("winston_item_hidden");
        this.sel_mode = 1;
    ...
    destroyCurrentExpander: function(){
        ...
        $(document.getElementById("winston_AEC")).addClass("winston_item_hidden");
        $(".winstondom_matched_element").removeClass("winstondom_matched_element");
    ...
    // Some of that is not as simple as it sounds.
    ...
    loadWinstonBaseListeners: function(){
        ...
        common_container.on('click.winstoncr', "#winston_base_back",function(){
                WinstonSocket.sendEvent({action:"Winston",target:"undo"});
                return false;
        });

        common_container.on('click.winstoncr',"#winston_base_forward",function(){
        ...
        common_container.on('click.winstoncr', "#winston_base_pause",function(){
                paused = !paused;
                if(paused){
                        destroyListeners(false);
                        $("b.winston_name").html("Winston[Paused]");
                        $("b#winston_pause").html("▶");
                }else{
                        loadListeners(false);
        ...
        common_container.on('click.winstoncr', "#winston_AEC",function(){
            Winston.sel_mode = 2;
    ...
    confirmCurrentState: function(){
        if(Winston.sel_mode == 0){ // Not doing shit
            ...
            if(Winston.active_expander.current_length == 1){
                ...
                Winston.active_expander.base[0].click(); //continue event
