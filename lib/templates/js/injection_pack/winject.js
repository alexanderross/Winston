...
var waiting_for_templates = false;

var WinPrefs = {
    //Load the core listeners before anything else (to beat the page's listeners)
    rush_those_listeners: true, 

    preload_templates: false,
    // Ignore the click event on an input. 
    // This is still handled by the simple dispatch mechanism, but it will resolve the element to it's name by default.
    // If disabled, clicking enables a unique selection state where input is entered, then a confirmation button on the panel is hit, then triggering
    //    resolution expanders to derive the actual selector that will be used.
    reject_input_clicks: true,
    seek_attrs: {
        "default":["class","id","_label"],
        "css_selector_based":{
            "input":["type","value"],
            ".example_class":["data-special"],
            "a":["href"]
        }
    }
...
var DRRenderer = {
   component_templates: undefined,
    ...
    fetchTemplates: function(){
        if(DRRenderer.templates_loaded == true) return true;
        WinstonSocket.sendEvent({action:"Winston",target:"load_templates"});
        DRRenderer.templates_loaded = true;
    },
    ...
    getValidAttrForElement: function(el){
        var attrs = WinPrefs.seek_attrs["default"];
        var scankeys = Object.keys(WinPrefs.seek_attrs["css_selector_based"]);
        for(var i in scankeys){
            if($(el).is(scankeys[i])){
                attrs = attrs.concat(WinPrefs.seek_attrs["css_selector_based"][scankeys[i]]);
            }
        }
        return  $.unique(attrs);
    },
   ...
   getNewBase: function(base_obj){
    var template = DRRenderer.component_templates["main"];
   ...
   getNewDomExpander: function(obj){
    var template = DRRenderer.component_templates["expander"];
   ...
   getNewExpanderAttrComp: function(attr_comp_val){
    var template = DRRenderer.component_templates["expander_attr_comp"];
   ...
   getNewExpanderAttr: function(attr){
    var template = DRRenderer.component_templates["expander_attr"];
    ...
    this.instantiateWrapper = function(parent_wrapper, attr, value){
       ...
       if(attr=="class"){
            ...
            for(var i=0;i<values.length;i++){
                //is used, value
                if(values[i] != "") this.attr_states.push([false, values[i]])
            }
       }else{
...
function WinDomWrapper (obj, parent_wrap, child_wrap) {
    ...
    this.instantiateWrapper = function(base_obj, parent_wrap, child_wrap){
        ...
        var tmp_attrs = DRRenderer.getValidAttrForElement(this.base);
...
var Winston = {
    ...
    loadAll: function(just_listeners){
        if(Winston.loaded) return true;
        if(DRRenderer.component_templates == undefined){
            if (!waiting_for_templates) DRRenderer.fetchTemplates();
            waiting_for_templates = true;
            console.log("Warning :: render templates not defined at load time. Requested templates and will attempt reload when templates get here.");
            return false;
        }
        ...
        if(!this.page_listeners_loaded) this.loadPageListeners();
    ...
    isValidObjectForEvent:function(object){
        var is_valid = (object.className.indexOf("winston_") == -1);
        is_valid &= (object.id.indexOf("winston_") == -1);
        is_valid &= (Winston.sel_mode == 0);
        if(WinPrefs.reject_input_clicks) is_valid &= ((object.tagName != "INPUT") && (object.tagName != "TEXTAREA") && (object.tagName != "SELECT"));
        return is_valid;
    },
    catchEvent:function(evt){
        ...
        if(this.isValidObjectForEvent(object)){
    ...
    loadPageListeners: function(){
        $(document).on("blur.winston", "input:not(input:checkbox),textarea",function(evt){//TextField
            //WinstonSocket.queueEvent(WinstonSocket.eventForAction("FieldBlur",this));
            WinstonSocket.sendEvent({action: "FieldBlur", target: evt.target.name,value:$(evt.target).val()});
        });

        $(document).on('change.winston', 'input[type="file"]',function(evt){
            //WinstonSocket.queueEvent(WinstonSocket.eventForAction("FileUpload",this));
            WinstonSocket.sendEvent({action: "FileUpload", target: evt.target.name,value:$(evt.target).val()});
        });

        $(document).on('change.winston', "input:checkbox", function(evt){
            if($(evt.target).is(':checked')){
                //WinstonSocket.queueEvent(WinstonSocket.eventForAction("CheckboxCheck",this));
                WinstonSocket.sendEvent({action: "CheckboxCheck", target: evt.target.name, value: $(evt.target).val()});
            } else {
                //WinstonSocket.queueEvent(WinstonSocket.eventForAction("CheckboxUnCheck",this));
                WinstonSocket.sendEvent({action: "CheckboxUnCheck", target: evt.target.name, value: $(evt.target).val()});
            }
        });

        $(document).on('change.winston',"select", function(evt){
            var $object = $(evt.target);
            var sval = $object.find("option:selected").html();
            WinstonSocket.sendEvent({action: "SelectChange", target: evt.target.name, value: sval});
    ...
    processSettings: function(msg){
        DRRenderer.component_templates = msg.templates;
        console.log("Templates Loaded");
        if(waiting_for_templates == true){
            active_engine.processUpdate(msg);
            waiting_for_templates=false;
        }
...
if(WinPrefs.rush_those_listeners){
    Winston.loadPageListeners();
}
$(document).bind("ready",function(){
    ...
    chrome.extension.onMessage.addListener(function(msg, sender, sendResponse) { 
        ...
        if (msg.swag == "settings"){
            active_engine.processSettings(msg);
        }
    ...
    if(WinPrefs.preload_templates == true) DRRenderer.fetchTemplates();
    WinstonSocket.sendEvent({action:"Winston",target:"what the fuck am i doing?"});
});